...
namespace StarterAssets {
	...
	public class ThirdPersonController : MonoBehaviour {
		...
		private void Start() {
			_hasAnimator = Anim.TryGetComponent(out _animator);
		...
		private void Move()
		{
			...
			if (_input.move != Vector2.zero) {
				...
				if(!aiming){
					...
					rotation = _mainCamera.transform.eulerAngles.y;
			...
			if (_hasAnimator) {
				...
				if (_input.move != Vector2.zero) {
					...
					_animator.SetFloat( _animIDMoveY , 0); 
					_animator.SetFloat( _animIDMoveX , 0);
				}
				*/
			}
		}
		...
		private void Shooting(){
			if(canShoot){
				if (_input.shoot){
					RaycastHit camHit;
					float camRayMaxLength = 100f;
					
					if(!Physics.Raycast(Camera.main.transform.position, Camera.main.transform.forward, out camHit, camRayMaxLength, mylayerMask )){
						camHit.point = Camera.main.transform.position + Camera.main.transform.forward * camRayMaxLength;
					}
					
					//Vector3 shotDirection = GetShotDirectionWithinSpread(ShootPoint);
					 
					//Transform shotDirection = 
					ShootPoint.LookAt(camHit.point);
					//ProjectileBase 
					GameObject newProjectile = Instantiate(ProjectilePrefab, ShootPoint.position, Quaternion.LookRotation(ShootPoint.forward));
					newProjectile.GetComponent<ProjectileBase>().OnShoot();
